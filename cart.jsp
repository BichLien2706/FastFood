...
<ul class="list-inline shop-badge badge-lists badge-icons pull-right">
	<li><a href="#"><i class="fa fa-shopping-cart"></i></a> <c:set
			...
			var="map">
			<c:set var="count" value="${count + map.quantity}" />
		</c:forEach> <span class="badge badge-sea rounded-x">${count }</span>

		<ul class="list-unstyled badge-open mCustomScrollbar"
			data-mcs-theme="minimal-dark">
			<c:forEach items="${sessionScope.cart}" var="cart">
				<li><c:url value="/image?fname=${cart.product.image }"
						var="imgUrl"></c:url> <img src="${imgUrl}" alt="" width="10"
					height="20"> 
					<a href="${pageContext.request.contextPath}/member/cart/remove?pId=${cart.id}"><button
					type="button" class="close">×</button></a>
					<div class="overflow-h">
						<span>${cart.product.name }</span> <small>${cart.quantity }
							* ${cart.product.price }.0 $ </small>
					</div></li>
			</c:forEach>
			...
			<li class="subtotal">
				<div class="overflow-h margin-bottom-10">
					...
					<c:forEach items="${sessionScope.cart}" var="map">
						<c:set var="total" value="${total + map.quantity * map.product.price}" />
					</c:forEach>
				...
				<div class="row">
					<div class="col-xs-6">
						<a href="${pageContext.request.contextPath}/member/cart" class="btn-u btn-brd btn-brd-hover btn-u-sea-shop btn-block">View Cart</a>
					</div>
					<div class="col-xs-6">
